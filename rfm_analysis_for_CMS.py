...
import definition_rm_heatmap as def_rm_heatmap
...
def extract_transaction_dataframe(json_data):
    transactions = json_data['data']['data']
    
    transaction_list = []
    for idx, trans in enumerate(transactions, start=1):
        date = trans['created_at'][:10]
        patient_id = f"{trans['patient']['firstname'].strip()} {trans['patient']['lastname'].strip()}"
        provider = trans['docter']['name']
        # revenue = trans['total']
        
        for service in trans['invoice_services']:
            treatment_name = service['packages']['name']
            revenue = service['packages']['price']
...
url = "https://api-dev.senyum.in/login"
...
def get_transactions():
    try:
        # response = requests.get(API_URL, timeout=10)  # optional headers or auth may be required
...
data_transactions = get_transactions()


# if data_transactions:
#     st.json(data_transactions)  # Just an example to visualize the response
# else:
...
st.title("Client RFM Analysis")
st.write("This analytics model helps to segment your customers based on their purchase behavior. "
        "The app uses the RFM model to segment customers into different categories based on their recency, frequency, "
        "and monetary value as the three key behavioral factors.")

st.markdown("**Recency (R)**: How recently a customer has made a purchase.<br>"
            "**Frequency (F)**: How often a customer makes a purchase within a given time period.<br>"
            "**Monetary (M)**: How much a customer spends over a given time period.",
            unsafe_allow_html=True)

# with st.container(border=True):

#     st.markdown("### Run your own Clinic Data")

#     st.write('To successfully run this app, please upload your own data in a format provided by the template data below.')

#     st.download_button(
#         label="Download Template Data",
#         data="File to Fill.xlsx",
#         file_name="File to Fill.xlsx",
#         mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
#     )
#     st.caption('Replace sample data with your own data in the same format as the template provided above. Note that blue columns are mandatory to fill, and green columns are optional to fill.')

#     st.write('Upload your data below:')
#     file_uploaded = None
#     file_uploaded = st.file_uploader("Upload your file", type=["xlsx", "csv"])
...
df = extract_transaction_dataframe(data_transactions)
...
# if column 'Revenue ($)' exist, rename it to 'Price'
if 'Revenue ($)' in df.columns:
    df.rename(columns={'Revenue ($)': 'Price'}, inplace=True)


col1, col2 = st.columns(2)

with col1:
    beginning_date = st.date_input("Select the beginning date", min_value=df['Date'].min().date(), max_value=df['Date'].max().date(), value=df['Date'].min().date())
    beginning_date = dt.datetime.combine(beginning_date, dt.datetime.min.time())
with col2:
    reference_date = st.date_input("Select the reference date", min_value=df['Date'].min().date(), max_value=df['Date'].max().date())
...
with st.popover("Understanding RFM Calculation"):
    def_rfm_table.app()
...
with col1:
    ...
    pivot_data = segment_counts.pivot(index="R Score", columns="M Score", values="Count").fillna(0).astype(int)
    ...
    st.markdown("### Recency-Monetary Segment Distribution Heatmap")
...
with col3:
    with st.popover("Understanding Recency-Monetary Heatmap"):
        def_rm_heatmap.app() 
        
with col4:
    with st.popover("Understanding Recency-Frequency Scatter Plot"):
        def_rf_scatter.app()
